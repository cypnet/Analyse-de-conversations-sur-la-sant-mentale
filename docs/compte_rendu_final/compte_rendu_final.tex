...
\titleformat{\chapter}[display]
  ...
  {10pt}                               % Espace entre l'étiquette et le titre
...
\titlespacing*{\chapter}
  ...
  {20pt}      % Espace vertical APRÈS le titre (réduit)
\onehalfspacing
...
\pagestyle{fancy}
\fancyhf{}
\lhead{\small Rapport Final - Analyse de conversations patient-thérapeute}
...
\begin{itemize}
    \item \textbf{Chef de Projet (Global) :} Logan LARROUX — garant de la cohérence globale, supervise les deux groupes, consolide les livrables.
    \item \textbf{Chef de Groupe 1 (Résumé conversations) :} Cruz Fernandes Diogo
    \item \textbf{Chef de Groupe 2 (Q/R santé mentale) :} Idir Yahiaoui
\end{itemize}
...
Tout les traitements qui compose l'étape 2 seront détaillés plus loin dans la Troisième partie du rapport. 
Si vous préféré avoir un condensé, celles-ci sont aussi présent dans le cahier des charges.
\end{note}
...
Chaque membre a adopté une stratégie de nettoyage plus ou moins stricte, ce qui a conduit à des résultats 
différents en termes de nombre de patients et de thérapeutes uniques, ainsi que de la longueur moyenne des conversations.
...
\midrule
995 & 8 personnes & \textbf{TODO} & Sans prétraitement \\
819 & 1 personne  & \textbf{TODO} & Prétraitement + filtre espagnol \\
796 & 1 personne  & \textbf{TODO} & Prétraitement + filtre espagnol \\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{Résultat} & \textbf{Nb. Membres} & \textbf{Nom Membres} & \textbf{Méthode} \\
\midrule
2 479 & 6 personnes & \textbf{TODO} & Sans prétraitement \\
2 472 & 1 personne  & \textbf{TODO} & Léger prétraitement \\
2 409 & 1 personne  & \textbf{TODO} & Prétraitement complet \\
$\geq 12$ & 1 personne & \textbf{TODO} & Pattern matching \\
\bottomrule
\end{tabular}
\end{center}

\begin{avertissement}
\textbf{Limite importante :} Les valeurs autour de 2 400--2 479 correspondent très probablement au nombre de lignes/réponses dans le fichier, et non au nombre réel de thérapeutes distincts. En l'absence d'identifiant unique (ID), il est impossible de différencier les thérapeutes entre eux avec certitude.
\end{avertissement}
...
Dans chaque cas, les thérapeutes ont tendance à produire des réponses plus longues que les patients, ce qui est cohérent avec la nature de leur rôle dans la conversation.

\bigbreak
Maintenant que nous avons une idée de la structure générale du corpus, nous allons pouvoir nous pencher plus en détail sur les thématiques abordées dans les conversations, 
...
Un dictionnaire thématique est construit manuellement, en associant chaque thème de santé mentale 
...
\chapter{Conclusion comparée des méthodes}
