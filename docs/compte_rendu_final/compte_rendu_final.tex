...
\begin{itemize}
  ...
  \item \textbf{Sentiment (BERT)}~: \textit{Suicidal}
\end{itemize}
...
\node[box, below=of qu] (vu) {Vectorisation\\question utilisateur};
\node[box, right=of vu] (ext) {Sentiment BERT};
\node[box, below=of qc, text width=4cm] (flt) {Filtrer selon sentiment BERT\\de la question};
\node[box, below=of flt] (vc) {Vectorisation\\corpus filtré};
...
\node[box, below=of qu] (vu) {Vectorisation\\question utilisateur};
\node[box, below=of qc] (vc) {Vectorisation\\réponses};
\node[box, below=2.5cm of qu, xshift=4cm] (s) {Similarité cosinus};
\node[box, below=of s] (t) {Tri des scores};
...
\node[box, right=of vu] (ext) {Sentiment BERT};
\node[box, below=of qc, text width=4cm] (flt) {Filtrer selon sentiment BERT\\de la question};
\node[box, below=of flt] (vc) {Vectorisation\\réponses filtrées};
\node[box, below=6cm of qu, xshift=4cm] (s) {Similarité cosinus};
\node[box, below=of s] (t) {Tri des scores};
\node[box, below=of t] (k) {Retour des $k$ plus proches réponses};
\draw[arrow] (qu) -- (vu);
...
L'interface est configurée en pleine largeur (\texttt{max-width: 100\%}) et
accessible sur mobile. Le score MRR de 0,702 est affiché dans l'en-tête de
...
problème algorithmique. C'est pour cela que le Modèle~B (BERT avec fine-tuning) a été retenu
