...
Après constatation de l'ampleur des tâches à réaliser lors de l'étape 1, une bien meilleure organisation a été mise en place pour l'étape 2, avec des chefs de groupe dédiés à chaque axe d'analyse.

\subsection{Organisation étape 1: Exploration du jeu de données}
Lors de l'étape 1 chaque membre de l'équipe s'est vue assigner les mêmes tâches, afin d'en comparer et de contraster les résultats.
...
En plus de cette séparation dans ces deux groupes, nous avons de nouveau séparé les tâches en sous-tâches, afin de mieux répartir le travail et d'assurer une meilleure coordination entre les membres.
...
manuellement, voici deux exemples de listes d'extra\_stopwords utilisées par Logan et Lucas:
...
Puisque ce pipeline n'a pas été imposé, chaque membre a pu appliquer son propre pré-traitement, avec des niveaux de filtrage différents.
Prenons l'exemple de deux approches contrastées, celle d'Emin et celle de Diogo.
...
Une version beaucoup plus étendue est celle de Diogo, qui inclut un nettoyage plus poussé du texte, 
...
L'application de la lemmatisation (et de la tokenisation) pour l'analyse du texte a été un débat au sein de l'équipe.
La grande majorité de l'équipe a décidé de ne pas l'appliquer lors de l'analyse statistique et lexicale du corpus, 
...
Par question de clarté, nous allons présenter les résultats de chaque méthode de manière synthétique, en présentant les résultats les plus pertinents et en discutant de leurs avantages et limites respectifs.\\
...
Or cela ne signifie pas que ces thématiques sont les plus fréquentes,
bien au contraire! D'autres thèmes bien plus spécifiques auxquels on ne penserait pas peuvent faire leur apparition. 
D'où l'intérêt de méthodes plus robustes et moins subjetives que le lexique manuel. 
...
Vectoriser un texte signifie le transformer en une représentation numérique, pour que par la suite les algorithmes puissent les traiter.\\
Pour cela, nous allons utiliser la fonction \texttt{TfidfVectorizer} de la bibliothèque \textbf{scikit-learn}.
Cette fonction a de grands avantages, elle permet de:
\begin{itemize}
...
Gràce à cette matrice, nous allons pouvoir maintenant regrouper les mots similaires en \textbf{clusters},
...
De plus, selon les membres, nous avons défini un nombre de thèmes $N\_TOPICS$ et un nombre de passes $PASSES$ pour l'entraînement du modèle.
\begin{figure}[ht]
  \begin{lstlisting}
    N_TOPICS = 6
    PASSES = 15
...
Les résultats obtenus avec LDA sont généralement plus interprétables que ceux obtenus avec les méthodes basées sur les embeddings, car ils fournissent une distribution de mots pour chaque thème. 
...
Cependant, cela nécessite une expertise plus approfondie et une validation rigoureuse pour éviter les biais et les interprétations erronées.
...
Ces résultats ont conduit à une revue complète de l'organisation pour la prochaine étape, avec une standardisation des résultats et une meilleure documentation pour faciliter la collaboration.
...
\textbf{Trois variantes de BERT} sont implémentées, dont l'une d'entre elles grandement privilégiée par l'équipe. 
...
\begin{note}[Remarque]
Chaque membre a décidé de sa propre volonté la méthode qu'il préférait implémenter. 
...
Ce choix a donc été fait en amont afin de définir les tâches à réaliser pour chaque membre de l'équipe.
\end{note}
...
Maintenant que nous avons mis en place les bases communes, nous pouvons détailler les implémentations de chaque modèle et les résultats obtenus.
...
\subsection{Résultats obtenus — Diogo}

Diogo s'étant occupé de cette méthode seul, les résultats obtenus sont les suivants:\\
...
\subsection{Résultats obtenus — Diogo, Ulysse, Victor et Emin}

Étant la méthode la plus apprécié par l'équipe, Diogo, Ulysse, Victor et Emin se sont occupé de 
celle-ci, les résultats obtenus sont les suivants:\\
...
\subsection{Résultats obtenus — Logan}

Contrairement à la précédente, celle-ci est un peu plus complexe et donc a été réalisée par Logan seul, les résultats obtenus sont les suivants:\\
...
Malgré une grande amélioration des classes difficiles (Personality disorder, Stress, Suicidal) avec les modèles BERT,
