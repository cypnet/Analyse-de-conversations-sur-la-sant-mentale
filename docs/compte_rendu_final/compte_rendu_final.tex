...
\usepackage{fancyhdr}
% Augmente la hauteur d'en-tête pour éviter les warnings de fancyhdr
...
\begin{titlepage}
  ...
  {Professeure à l'Université de Toulouse\\
  Membre de l'Institut de Recherche en Informatique de Toulouse (IRIT) }
  ...
  \begin{tabular}[t]{c}
    ...
    \hspace{10px}
    \raisebox{0.70cm}{\normalsize [Licence Informatique]\par}
    \hspace{2.5px}
...
\item \href{https://huggingface.co/spaces/dempxl/sentiment-analysis-for-mental-health}{Page Hugging Face du projet}
\end{itemize}
...
% ------------------------------------------------------------------------------
\subsection{Méthode supervisée --- Naive Bayes, Logistic Regression et LinearSVC}
% ------------------------------------------------------------------------------
...
Logan a produit le notebook de base structurant le travail commun. Sa contribution individuelle principale est l'implémentation d'une \textbf{Régression Logistique} (LR + TF-IDF optimisé).
...
Le sous-groupe non-supervisé a comparé deux approches de vectorisation~: \textbf{Word2Vec} et \textbf{TF-IDF + SVD}.
...
\begin{figure}[H]
  ...
  \caption{Matrice de confusion --- K-Means final (TF-IDF)}
\end{figure}
...
\toprule
\textbf{Critère} & \textbf{LR TF-IDF } & \textbf{K-Means TF-IDF} \\
                 & \textbf{(supervisé)} & \textbf{(non-supervisé)} \\
\midrule
\textbf{Accuracy test}          & \textbf{0.7532} & 0.4636 \\
Macro avg F1                    & 0.72            & ---    \\
Weighted avg F1                 & 0.75            & ---    \\
Voit les labels                 &                 &    \\
 à l'entraînement ?             & Oui             & Non    \\
Hyperparamètre clé              & alpha = 0.07    & $k = 7$ \\
Fichier annoté                  & \texttt{train\_with\_\newline sentiments.csv} & \texttt{train\_with\_\newline sentiments\_kmeans.csv} \\
\bottomrule
\end{tabular}
\end{center}

L'écart de \textbf{+29 points d'accuracy} en faveur du supervisé est cohérent et attendu~: le supervisé a accès aux vrais labels pendant l'entraînement, le non-supervisé pas. Les deux méthodes utilisent la même représentation TF-IDF, donc l'écart reflète purement la différence supervisé/non-supervisé.
...
AdamW est utilisé à la place d'Adam classique, car il corrige la gestion du \textit{weight decay}\footnote{\textit{[\ldots] 'Loshchilov and Hutter 2017 discovered that L2 regularization is not effective in Adam, and proposed AdamW' [\ldots]}
...
les hyperparamètres recommandés étant issus de la section 3 du papier \href{https://arxiv.org/pdf/1810.04805}{"BERT: Pre-training of Deep Bidirectional Transformers for
...
et est aujourd'hui une base très solide dans l'analyse de sentiments.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.65\textwidth]{matriceConfusion_BERT_PT_FT_LOGAN.png}
...
\subsubsection{5. Conclusion et choix de modèle}
En conclusion, le modèle \textbf{BERT spécialisé avec fine-tuning (Modèle C) offre la meilleure performance}, même si le gain par rapport au Modèle B est marginal.
Pour la suite du projet, nous allons donc utiliser le modèle C pour générer les annotations de sentiment sur le dataset principal \texttt{train.csv}, 
qui seront ensuite réutilisées dans le système de question-réponse du Module~2 pour filtrer les résultats par sentiment.

\textbf{Fichier généré:} \texttt{train\_with\_sentiments.csv}.
...
\midrule
Similarité question-réponse    & Cosine similarity & \\
...
Les annotations automatiques de sentiments produites par le Groupe~1 sont réutilisées ici pour \textbf{filtrer l'espace de recherche}. Suite à la comparaison des modèles du Module~1, le système utilise désormais la colonne \texttt{predicted\_sentiment\_bert} — issue du meilleur modèle BERT avec fine-tuning (accuracy 0,8343) — plutôt que les annotations TF-IDF initialement utilisées. À une question donnée, le système restreint sa recherche aux échanges partageant la même étiquette de sentiment BERT, assurant une cohérence thématique et émotionnelle de meilleure qualité.
\end{note}
...
(\texttt{train\_unique\_e2p2.csv}) en supprimant les doublons de paires (question, réponse). Nous
avons également constitué un ensemble de test en sélectionnant aléatoirement dix paires
issues du jeu de données (\texttt{test.csv}). Enfin, un module commun de prétraitement a été développé afin de nettoyer les données (mise en minuscule, suppression de la
...
\begin{itemize}
  \item \textbf{Context}~: \textit{"He was in love with someone years ago, and he still thinks about her time to time. He said, and I quote, ``That relationship is definitely over. I love you, but that girl will always be in my mind.'' It just didn't feel like he appreciated all the things I've done to make him happy."}
  \item \textbf{Response}~: \textit{"Trust your intuition on your conclusion about this guy. He may very well love you, only with the ex so prominent in his mind, it is possible your feeling of not being appreciated now, would multiply if ever the two of you needed to address a delicate topic."}
  \item \textbf{Sentiment (BERT)}~: \textit{Normal}
\end{itemize}
...
\subsubsection{Détails de vectorisation}
...
\begin{itemize}
    \item \textbf{TF-IDF} (Idir, Yvan)~: méthode statistique qui attribue un poids aux mots en fonction de leur fréquence dans un document et de leur rareté dans le corpus. Elle met en avant les termes les plus discriminants.
    \item \textbf{Word2Vec} (Laure, Trésor)~: chaque mot est transformé en vecteur numérique dans un espace continu. La représentation d'une phrase est obtenue en calculant la moyenne des vecteurs des mots qui la composent, capturant une similarité sémantique entre les textes.
    \item \textbf{BERT (\texttt{all-MiniLM-L6-v2})} (Idir, Yvan)~: modèle de langage basé sur les Transformers qui génère des représentations contextuelles des phrases, en tenant compte du sens des mots selon leur contexte global.
\end{itemize}
...
Afin d'obtenir une mesure globale de performance, nous avons calculé la moyenne des scores de similarité obtenus sur l'ensemble du jeu de test, en nous inspirant du principe du MRR (\textit{Mean Reciprocal Rank})~:

\[
\text{Score} = \frac{1}{N} \sum_{i=1}^{N} \text{cosine\_similarity}(R^{\text{pred}}_i,\; R^{\text{true}}_i)
\]

Bien que cette méthode s’inspire du principe du MRR (Mean Reciprocal Rank), elle repose ici directement
sur les scores de similarité et non sur un classement explicite des réponses.

Dans ce contexte, un score proche de 1 indique une bonne performance du modèle, 
car cela signifie que la similarité entre les réponses est élevée.
...
\begin{tikzpicture}[
  ...
  >=Latex,
  box/.style={rounded corners, draw=blue!70!black, thick, fill=blue!5,
              align=center, text width=2.2cm, minimum height=1.2cm,
              inner sep=6pt, font=\small},
  arrow/.style={-Latex, thick, draw=blue!70!black},
]
% Ligne du haut : côté question utilisateur
\node[box] (qu) {Question utilisateur};
\node[box, right=1.5cm of qu] (vu) {Vectorisation\\question};

% Ligne du bas : côté corpus
\node[box, below=1.8cm of qu] (qc) {Questions corpus};
\node[box, right=1.5cm of qc] (vc) {Vectorisation\\corpus};

% Suite commune (centrée verticalement entre les deux lignes)
\node[box, right=1cm of vu, yshift=-1.5cm] (s) {Similarité cosinus};
\node[box, right=1cm of s] (t) {Tri des scores};
\node[box, right=1cm of t] (k) {Sélection des $k$ plus proches};
\node[box, right=1cm of k] (r) {Retour des réponses};
...
\node[box] (qu) {Question utilisateur};
\node[box, right=1.5cm of qu] (vu) {Vectorisation\\question};
\node[box, below=1cm of vu, xshift=-1.8cm, draw=green!50!black, fill=green!5] (ext) {Sentiment BERT};

% Ligne du bas : côté corpus
\node[box, below=3.2cm of qu] (qc) {Questions corpus};
\node[box, right=1.2cm of qc] (flt) {Filtrage par sentiment BERT};
\node[box, right=1.2cm of flt] (vc) {Vectorisation\\corpus filtré};

% Suite commune
\node[box, right=1cm of vu, yshift=-1.95cm] (s) {Similarité cosinus};
\node[box, right=1.2cm of s] (t) {Tri des scores};
\node[box, right=1.2cm of t] (k) {Sélection des $k$ plus proches};
\node[box, right=1.2cm of k] (r) {Retour des réponses};

% Flèches côté question
\draw[arrow] (qu) -- (vu);
\draw[arrow, draw=green!50!black] (qu) |- (ext);
\draw[arrow, draw=green!50!black] (ext) -| (flt);
...
\end{tikzpicture}
\end{center}
\caption{Schéma de la méthode de similarité question-question (version 2 : filtrage par sentiment BERT).}
\end{figure}

\newpage

\textbf{Méthode TF-IDF}
...
\begin{tabular}{@{}lc@{}}
  ...
  Version & Score \\
  \midrule
  Sans filtrage & 0,3141 \\
  Avec filtrage  & \textbf{0,4101} \\
  \bottomrule
\end{tabular}
\end{center}
\caption{Scores de similarité question-question avec TF-IDF, avant et après filtrage par sentiment.}
\end{table}

\begin{note}[Interprétation des résultats]
L'amélioration du score de similarité après filtrage par sentiment BERT est significative, passant de 0,3141 à 0,4101.
Cela confirme que restreindre la recherche aux questions partageant le même sentiment BERT améliore la pertinence des réponses retrouvées, en assurant une cohérence émotionnelle entre la question d'entrée et les questions du corpus.
\end{note}

\pagebreak

\textbf{Méthode Word2Vec}

Chaque question est représentée par la \textbf{moyenne} des vecteurs de ses mots, tels qu'appris par un modèle Word2Vec entraîné sur le corpus (\texttt{vector\_size=100}, \texttt{window=5}, \texttt{min\_count=1}).
...
\begin{tabular}{@{}lc@{}}
  ...
  Version & Score \\
  \midrule
  Sans filtrage & 0,4242 \\
  Avec filtrage  & \textbf{0,6220} \\
  \bottomrule
\end{tabular}
\end{center}
\caption{Scores de similarité question-question avec Word2Vec, avant et après filtrage par sentiment.}
\end{table}

\begin{note}[Interprétation des résultats]
L'approche Word2Vec améliore significativement les scores de similarité par rapport à TF-IDF, avec un gain de plus de 10 points après filtrage par sentiment (0,6220 contre 0,4101). 
Cela s'explique par la capacité de Word2Vec à capturer les relations sémantiques entre les mots, permettant ainsi une meilleure représentation des questions et une similarité plus pertinente.
\end{note}

\pagebreak

\textbf{Méthode BERT (\texttt{all-MiniLM-L6-v2})}

Le modèle \texttt{all-MiniLM-L6-v2} génère des \textit{embeddings} contextuels pour chaque question, capturant le sens global de la phrase de manière bien plus riche que TF-IDF ou Word2Vec.
...
\begin{tabular}{@{}lc@{}}
  ...
  Version & Score \\
  \midrule
  Sans filtrage & 0,4627 \\
  Avec filtrage  & \textbf{0,6700} \\
  \bottomrule
\end{tabular}
\end{center}
\caption{Scores de similarité question-question avec BERT, avant et après filtrage par sentiment.}
\end{table}

\begin{note}[Interprétation des résultats]
L'utilisation de BERT améliore encore les scores de similarité, avec un gain de près de 5 points par rapport à Word2Vec après filtrage (0,6700 contre 0,6220). 
Cela s'explique par la capacité de BERT à générer des représentations contextuelles plus riches, prenant en compte les nuances de sens et les relations complexes entre les mots dans une phrase, 
ce qui se traduit par une meilleure identification des questions similaires.
\end{note}

\vspace{0.5cm}
Maintenant que nous avons présenté la première approche basée sur la similarité entre questions, nous allons détailler la seconde approche qui compare directement la question de l'utilisateur avec les réponses du corpus.
...
% Ligne du bas : côté corpus
\node[box, below=1.8cm of qu] (qc) {Réponses corpus};
\node[box, right=1.5cm of qc] (vc) {Vectorisation\\réponses};

% Suite commune
\node[box, right=1cm of vu, yshift=-1.5cm] (s) {Similarité cosinus};
\node[box, right=1cm of s] (t) {Tri des scores};
\node[box, right=1cm of t] (k) {Retour des $k$ plus proches réponses};
...
% Ligne du bas : côté corpus
\node[box, below=3.2cm of qu] (qc) {Réponses corpus};
\node[box, right=1.2cm of qc] (flt) {Filtrage par sentiment BERT};
\node[box, right=1.2cm of flt] (vc) {Vectorisation\\réponses filtrées};

% Suite commune
\node[box, right=1cm of vu, yshift=-1.95cm] (s) {Similarité cosinus};
\node[box, right=1.2cm of s] (t) {Tri des scores};
\node[box, right=1.2cm of t] (k) {Retour des $k$ plus proches réponses};
...
\end{tikzpicture}
\end{center}
\caption{Schéma de la méthode de similarité question-réponse (version 2 : filtrage par sentiment BERT).}
\end{figure}
...
Chaque réponse du corpus est représentée par un vecteur TF-IDF. La similarité cosinus est calculée entre la question utilisateur et toutes les \textbf{réponses} appartenant au même sentiment BERT.
...
\begin{tabular}{@{}lc@{}}
  ...
  Version & Score \\
  \midrule
  Sans filtrage & 0,3200 \\
  Avec filtrage  & \textbf{0,4101} \\
  \bottomrule
\end{tabular}
\end{center}
\caption{Scores de similarité question-réponse avec TF-IDF, avant et après filtrage par sentiment.}
\end{table}
...
Chaque réponse est représentée par la moyenne des vecteurs de ses mots, issus d'un modèle Word2Vec 
entraîné cette fois sur les \textbf{réponses} du corpus (\texttt{vector\_size=100}, \texttt{window=5},
 \texttt{min\_count=2}).

\begin{lstlisting}
model_w2v2 = Word2Vec(sentences=phrases_responses, vector_size=100,
...
\begin{tabular}{@{}lc@{}}
  ...
  Version & Score \\
  \midrule
  Sans filtrage & 0,5300 \\
  Avec filtrage  & \textbf{0,6640} \\
  \bottomrule
\end{tabular}
\end{center}
\caption{Scores de similarité question-réponse avec Word2Vec, avant et après filtrage par sentiment.}
\end{table}
...
Le modèle \texttt{all-MiniLM-L6-v2} génère des \textit{embeddings} contextuels, 
appliqués ici aux \textbf{réponses} du corpus. La question utilisateur est encodée dans le même espace,
 permettant une comparaison sémantique directe.
...
\begin{tabular}{@{}lc@{}}
  ...
  Version & Score \\
  \midrule
  Sans filtrage & 0,5627 \\
  Avec filtrage  & \textbf{0,7020} \\
  \bottomrule
\end{tabular}
\end{center}
\caption{Scores de similarité question-réponse avec BERT, avant et après filtrage par sentiment.}
\end{table}
...
\begin{tabular}{@{}L{5.5cm}C{2.5cm}C{2.5cm}C{2.5cm}@{}}
  ...
  Similarité question-réponse  & 0,4101 & 0,6640 & \textbf{0,7020} \\
  \bottomrule
\end{tabular}
\end{center}
...
\begin{note}[Apport du filtrage par sentiment BERT]
Le passage au filtrage via \texttt{predicted\_sentiment\_bert} (meilleur modèle du Module~1, accuracy 0,8302) 
apporte un gain systématique sur toutes les combinaisons. La stratégie optimale reste 
\textbf{BERT + similarité question-réponse + filtrage BERT}, avec un score de \textbf{0,702}. 
La qualité supérieure des annotations BERT, par rapport aux annotations TF-IDF initialement utilisées, 
se traduit directement par des sous-ensembles de recherche plus cohérents émotionnellement.
\end{note}

\pagebreak

% ══════════════════════════════════════════════════════════════════════════════
% CHAPITRE 13 : DÉPLOIEMENT HUGGING FACE
% ══════════════════════════════════════════════════════════════════════════════
\chapter{Déploiement — Application Hugging Face Spaces}

\section{Présentation générale}

L'ensemble du pipeline développé au cours de ce projet a été intégré dans une
application web interactive et déployée publiquement sur la plateforme
\textbf{Hugging Face Spaces}~:

\begin{center}
\url{https://huggingface.co/spaces/dempxl/sentiment-analysis-for-mental-health}
\end{center}

Cette application constitue la démonstration concrète et interactive du travail réalisé.
Elle réunit en un seul endroit les deux modules de l'Étape~2~: la classification de
sentiments et le système de suggestion de réponses, permettant de les tester sur
n'importe quelle phrase en langage naturel (de préférence an anglais).

\begin{warn}[Avertissement éthique]
Les résultats produits par l'application sont \textbf{indicatifs et non médicaux}. 
L'application affiche explicitement ce rappel à l'utilisateur~: elle ne constitue en aucun cas un diagnostic clinique.
\end{warn}

\section{Architecture technique}

\subsection{Stack et dépendances}

L'application repose sur \textbf{Gradio} pour l'interface, et mobilise les bibliothèques
suivantes~:
...
\midrule
\texttt{gradio}               & Interface web interactive \\
\texttt{transformers}         & Chargement du modèle DistilRoBERTa fine-tuné \\
\texttt{sentence-transformers} & Encodage BERT pour le système Q-R \\
\texttt{torch}                & Inférence du modèle de classification \\
\texttt{scikit-learn}         & Modèle LR + TF-IDF et calcul de similarité cosinus \\
\texttt{nltk}                 & Tokenisation et stemming (prétraitement LR) \\
\texttt{pandas}, \texttt{numpy} & Manipulation des données \\
\bottomrule
\end{tabular}
\end{center}

\subsection{Chargement des modèles}

Au démarrage, l'application charge deux classifieurs en parallèle~:

\begin{itemize}
\item \textbf{Modèle LR + TF-IDF} — désérialisé depuis des fichiers \texttt{.pkl}
\item \textbf{Modèle BERT fine-tuné} — avec détection automatique de la source (checkpoint local vs. Hugging Face Hub en fallback)
\end{itemize}

\begin{tip}[Robustesse au LFS]
La fonction \texttt{ensure\_not\_lfs\_pointer()} vérifie que les fichiers du checkpoint
ne sont pas de simples pointeurs Git-LFS vides (première ligne~: 
\texttt{version https://git-lfs.github.com/spec/v1}). Si c'est le cas, le modèle
est téléchargé depuis le Hub Hugging Face plutôt que chargé localement, garantissant
...
\begin{itemize}
\item \textbf{Pour LR + TF-IDF} — nettoyage puis stemming Porter
\item \textbf{Pour BERT} — seul \texttt{cleaner()} est appliqué, le tokeniseur \texttt{AutoTokenizer} gère ensuite la segmentation en sous-mots (WordPiece).
\end{itemize}

\section{Module 1 dans l'application — Comparaison des classifieurs}

L'onglet \textbf{«~Comparaison des classifieurs~»} permet de soumettre un texte et
d'observer côte à côte les prédictions des deux modèles, avec les probabilités pour
chacune des 7 classes.

\subsection{Prédiction LR + TF-IDF}

\begin{lstlisting}
def predict_lr(text: str):
# 1. Nettoyage + stemming
# 2. Vectorisation TF-IDF
# 3. Prédiction des probabilités
# 4. Récupération de l'ordre des classes
# 5. Construction du dictionnaire classe:probabilité
# 6. Classe avec la probabilité maximale
# 7. Retour du dictionnaire et de la classe dominante formatée en gras
\end{lstlisting}

\subsection{Prédiction BERT fine-tuné}

\begin{lstlisting}
def predict_bert_clf(text: str):
# 1. Nettoyage de base (sans stemming)
# 2. Tokenization
# 3. Passage sur le même device que le modèle
# 4. Inférence sans calcul de gradients
# 5. Récupération des logits bruts
# 6. Softmax pour obtenir les probabilités
# 7. Récupération de l'ordre des classes
# 8. Construction du dictionnaire classe:probabilité
# 9. Identification de la classe dominante
# 10. Classe avec la probabilité maximale
# 11. Retour du dictionnaire et de la classe dominante formatée en gras
\end{lstlisting}

\subsection{Exemples de prédictions}

Les tableaux suivants présentent les probabilités retournées par chaque modèle sur
quatre phrases représentatives des catégories du corpus.

\bigskip

\textbf{Exemple 1} — \textit{"I feel so anxious all the time, my heart races and I can't
stop worrying about everything."}

\begin{center}
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{Classe} & \textbf{LR + TF-IDF} & \textbf{BERT fine-tuné} \\
\midrule
\textbf{Anxiety}       & \textbf{0,6821} & \textbf{0,7934} \\
...
\midrule
\textbf{Dominant} & Anxiety & Anxiety \\
\bottomrule
\end{tabular}
\end{center}

\begin{note}[Interprétation]
Les deux modèles convergent vers \textit{Anxiety} avec une forte confiance. BERT est plus
tranché (0,793 vs 0,682), ce qui traduit sa meilleure capacité à capturer le contexte
émotionnel global de la phrase.
\end{note}

\pagebreak

\textbf{Exemple 2} — \textit{"I feel empty, like nothing matters anymore and nobody would
...
\textbf{Suicidal}      & 0,3467          & \textbf{0,6238} \\
...
\midrule
\textbf{Dominant} & Depression & Suicidal \\
\bottomrule
\end{tabular}
\end{center}

\begin{note}[Interprétation]
Ce cas illustre parfaitement la confusion \textit{Depression}/\textit{Suicidal} observée
durant l'évaluation~: LR classe \textit{Depression} en tête (0,391 vs 0,347) alors que
BERT détecte correctement \textit{Suicidal} avec une confiance bien plus élevée (0,624).
C'est précisément sur ce type de distinction nuancée que la compréhension contextuelle
de BERT fait la différence.
\end{note}

\bigskip

\textbf{Exemple 3} — \textit{"I've been feeling unusually energetic, I haven't slept in
days but I feel absolutely amazing and invincible."}
...
\textbf{Classe} & \textbf{LR + TF-IDF} & \textbf{BERT fine-tuné} \\
\midrule
\textbf{Bipolar}       & \textbf{0,4912} & \textbf{0,8103} \\
...
\midrule
\textbf{Dominant} & Bipolar & Bipolar \\
\bottomrule
\end{tabular}
\end{center}

\begin{note}[Interprétation]
Les deux modèles s'accordent sur \textit{Bipolar}. BERT capture avec une très haute
confiance (0,810) les marqueurs caractéristiques d'un épisode maniaque : énergie
excessive, manque de sommeil vécu positivement et sentiment d'invincibilité.
\end{note}

\bigskip

\textbf{Exemple 4} — \textit{"Work is suffocating me, I'm burning out and feel completely
...
\textbf{Classe} & \textbf{LR + TF-IDF} & \textbf{BERT fine-tuné} \\
\midrule
\textbf{Stress}        & \textbf{0,5231} & \textbf{0,6712} \\
...
\midrule
\textbf{Dominant} & Stress & Stress \\
\bottomrule
\end{tabular}
\end{center}

\section{Module 2 dans l'application — Suggestions de réponses}

L'onglet \textbf{«~Démo — Suggestions de réponses~»} implémente le pipeline Q-R complet
avec deux optimisations clés pour une utilisation en production~: le pré-calcul des
embeddings et un système de cache disque.

\subsection{Construction de l'index Q-R avec cache disque}

Le pré-calcul des embeddings de toutes les réponses du corpus est réalisé une seule
fois au démarrage, puis mis en cache sur disque pour les redémarrages suivants~:

\begin{lstlisting}
def _build_qr_index():
    # 1. Chargement du CSV annote
    # 2. Initialisation du modèle d'encodage BERT (all-MiniLM-L6-v2)
    # 3. Groupement par sentiment et encodage par bucket
        # 3.1. Utilise le cache si les embeddings sont plus recents
        # que le CSV (invalidation par mtime)
        # 3.2. Stockage en RAM pour accès rapide lors des requêtes
\end{lstlisting}

\begin{tip}[Stratégie de cache]
Le cache est invalidé automatiquement si le fichier CSV source est plus récent que
les embeddings sauvegardés (comparaison par \texttt{mtime}). À chaque redémarrage
sans modification des données, le chargement est quasi-instantané.
\end{tip}

\subsection{Pipeline de suggestion à la volée}

Lors d'une requête utilisateur, seule la phrase d'entrée est encodée à la volée.
Les embeddings du corpus étant pré-calculés, la latence est réduite à quelques
millisecondes~:
...
\begin{note}[Découplage des modèles de filtrage]
Un choix de conception notable~: le sentiment est détecté via LR + TF-IDF (rapide,
pas de GPU requis) pour filtrer le bucket, tandis que l'encodage sémantique de la
requête utilise BERT (\texttt{all-MiniLM-L6-v2}). Ce découplage permet d'obtenir
un temps de réponse quasi-immédiat tout en conservant la richesse sémantique de
BERT pour le calcul de similarité final.
\end{note}

\pagebreak

\subsection{Exemples de suggestions de réponses}

Les exemples suivants illustrent les résultats retournés par le système pour deux
phrases représentatives.

\bigskip

\textbf{Requête} — \textit{"My anxiety is getting worse every day, I panic in crowded
places and can't breathe."}

\begin{itemize}
  \item Sentiment détecté~: \textbf{Anxiety}
  \item Bucket~: réponses étiquetées \textit{Anxiety} dans le corpus
\end{itemize}

\begin{center}
\begin{tabular}{@{}clL{9cm}@{}}
\toprule
\textbf{\#} & \textbf{Score cosinus} & \textbf{Extrait de la réponse suggérée} \\
\midrule
1 & 0,6841 & \textit{"Panic attacks are very treatable. Many people find
relief through therapy (especially CBT), breathing exercises, and gradual
exposure to feared situations. Have you been able to talk to a professional
about what you're experiencing?"} \\
2 & 0,6312 & \textit{"It sounds like you're dealing with significant anxiety.
Learning to manage your breathing during moments of panic can be very
helpful — breathing slowly through the nose for 4 counts, holding for 4,
then exhaling for 4 tends to activate the parasympathetic nervous system."} \\
3 & 0,5987 & \textit{"Social anxiety can be incredibly isolating. What you're
describing — the physical sensations of panic in crowded places — are very
common symptoms. A therapist can help you work through this at your own
pace."} \\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{Requête} — \textit{"Sometimes I feel like two different people and I can't control
my reactions or emotions at all."}

\begin{itemize}
  \item Sentiment détecté~: \textbf{Personality disorder}
  \item Bucket~: réponses étiquetées \textit{Personality disorder} dans le corpus
\end{itemize}
...
\textbf{\#} & \textbf{Score cosinus} & \textbf{Extrait de la réponse suggérée} \\
\midrule
1 & 0,5923 & \textit{"Feeling like different parts of yourself are in conflict
is more common than you might think. It can reflect emotional dysregulation,
which is very treatable — Dialectical Behaviour Therapy (DBT) in particular
was designed for exactly this kind of difficulty."} \\
2 & 0,5541 & \textit{"The inability to control reactions can feel very
frightening and isolating. It may be worth exploring with a therapist whether
there are underlying patterns — often these responses developed as coping
mechanisms that no longer serve us."} \\
3 & 0,5102 & \textit{"Identity confusion and emotional volatility can have
many roots. A professional evaluation could help clarify what's happening and
...
\section{Interface Gradio et accessibilité}
...
\midrule
Démo — Suggestions de réponses
  & Zone de texte libre, curseur $k$ (1--10 suggestions),
    exemples prédéfinis chargeables en un clic.\newline
    Affiche le sentiment détecté, la taille du bucket et
    les $k$ réponses avec leur score et une barre visuelle. \\
\midrule
Comparaison des classifieurs
  & Zone de texte partagée, deux colonnes~: LR + TF-IDF et
    DistilRoBERTa fine-tuné.\newline
    Distributions de probabilité sur les 7 classes pour chaque modèle. \\
\midrule
À propos
  & Informations sur le projet, taille de l'index Q-R chargé. \\
\bottomrule
\end{tabular}
\end{center}

Le score MRR de 0,702 est affiché dans l'en-tête de
l'application pour transparence.
...
fine-tuné (Modèle~B) est marginal ($+0{,}4$ pt)}~: le fine-tuning sur notre dataset représente
l'essentiel de l'adaptation. Troisièmement, \textbf{la confusion \textit{Depression}/\textit{Suicidal}
persiste quel que soit le modèle}, ce qui pointe vers une limite des données plutôt qu'un
problème algorithmique. C'est pour cela que le Modèle~C (BERT spécialisé avec fine-tuning) a été retenu
comme source des annotations pour le Module~2.

\subsection{Le filtrage par sentiment BERT est le principal levier du système Q/R}
...
recherche par filtrage émotionnel. Le remplacement des annotations TF-IDF par les
annotations BERT (\texttt{predicted\_sentiment\_bert}) a permis d'obtenir des sous-ensembles
de recherche plus cohérents, amplifiant les bénéfices du filtrage.

\begin{center}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{@{}lcccc@{}}
\toprule
 & \multicolumn{2}{c}{\textbf{Sans filtrage}} & \multicolumn{2}{c}{\textbf{Avec filtrage BERT}} \\
...
La stratégie Q-R surpasse systématiquement la stratégie Q-Q, et le filtrage par sentiment BERT
apporte un gain plus important que le passage de TF-IDF à BERT sans filtrage.
...
\begin{itemize}
  \item \textbf{Pipeline unifié}~: fusionner le classificateur de sentiments et le système
    Q/R en un seul service, où l'annotation émotionnelle BERT est calculée à la volée pour
    chaque requête entrante.
  \item \textbf{Interface utilisateur}~: développer une application web (externe à HuggingFace)
    permettant à un professionnel de santé de soumettre une question et d'obtenir instantanément les
  ...
  \item \textbf{Apprentissage continu}~: mettre en place un mécanisme de retour utilisateur
    pour réentraîner périodiquement les modèles sur les nouvelles interactions et améliorer
    progressivement la pertinence des réponses.
\end{itemize}
...
pour le système Q/R avec filtrage par sentiment BERT~--- constituent une base sérieuse
pour des développements futurs orientés vers des applications cliniques réelles.
\end{note}

\vfill

\begin{center}
\textit{Merci à tous les membres de l'équipe pour leur travail acharné, leur rigueur et leur esprit de collaboration tout au long de ce projet.
Ce projet a été une aventure d'apprentissage collective, et les résultats obtenus sont le fruit de l'effort combiné de chacun. Continuons à construire sur cette base solide pour explorer de nouvelles frontières du NLP appliqué à la santé mentale !}

\textit{Merci pour votre lecture.}
\end{center}
...
\begin{itemize}
  ...
  \item Hugging Face — Projet fonctionnel\\
    \url{https://huggingface.co/spaces/dempxl/sentiment-analysis-for-mental-health}
