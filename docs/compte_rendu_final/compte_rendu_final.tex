...
\subsection{Indexation inversée}
...
Le code ci-dessous montre illustre le procesus de prétraitement des données : 
\begin{lstlisting}
...
afin de former un ensemble unique de mots représentatifs de la discussion. Ensuite, un
...
\begin{center}
    \(\text{mot} \rightarrow \{(\text{question}, \text{réponse})\}\)
\end{center}
...
\end{lstlisting}
\newpage
\subsection{Implémentation du système question-réponse}
...
\node[box, below=of qc] (flt) {Filtrer selon thème\\ de la question entrée};
...
Dans ce travail, plusieurs approches de vectorisation sont utilisées afin de représenter les questions et réponses dans un espace vectoriel commun. L’objectif est de capturer différents niveaux d’information, allant de la similarité lexicale à la similarité sémantique.

Afin de transformer les données textuelles en représentations exploitables par le modèle, plusieurs méthodes de vectorisation ont été étudiées et mises en œuvre.

\begin{itemize}
    \item \textbf{TF-IDF : }C'est une méthode statistique de représentation des textes qui attribue un poids aux mots en fonction de leur fréquence dans un document et de leur rareté dans le corpus. Elle permet de mettre en avant les termes les plus discriminants.
    \item \textbf{Word2Vec : }C'est une méthode de représentation vectorielle dans laquelle chaque mot est transformé en vecteur numérique dans un espace continu. La représentation d’une phrase est ensuite obtenue en calculant la moyenne des vecteurs des mots qui la composent, ce qui permet de capturer une similarité sémantique entre les textes.
    \newpage
    \item \textbf{BERT (all-MiniLM-L6-v2) : }BERT est un modèle de langage basé sur les Transformers qui génère des représentations contextuelles des phrases, en tenant compte du sens des mots selon leur contexte global.
\end{itemize}
...
\begin{itemize}
  ...
  \item  \textbf{Interface utilisateur} : Développer une application web ou mobile afin de de faciliter l’interaction avec le système et d’améliorer l’expérience utilisateur. 
\end{itemize}
